...
with tf.variable_scope('input'):
    # None means neural networks can mix up batches of any size
...
with tf.Session() as session:
    ...
    for epoch in range(training_epochs):
        ...
        if epoch % 5 == 0:
            ...
            # session runs more than one operations at the same time, more efficient
    ...
    # open tensorboard in terminal, navigate to the folder
    # tensorboard --logdir=./logs

    save_path = saver.save(session, "logs/trained_model.ckpt")
    print("Model saved: {}".format(save_path))

    # export model in production, e.g., google cloud platform
